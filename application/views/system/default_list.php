...
if(ISSET($data['records']) && sizeof($data['records'])>0){
	...
	echo '<tr>';
	echo '<th style="padding:0;"><input type="checkbox" id="chkAll" name="chkAll" class="form_chk_parent"></th>';
	echo '<th>No</th>';
	foreach($data['header'] as $header){
		$temp = $header['desc'];
	...
	foreach($data['records'] as $rec){		
		...
		if(!(ISSET($rec['data']['core']) && $rec['data']['core']==1)){
			echo '<td><input type="checkbox" name="chk_'.$rec['data']['id'].'" class="form_chk_node"></td>';
		}else{
			echo '<td>&nbsp;</td>';
		}		
		echo '<td>'.$count.'</td>';
